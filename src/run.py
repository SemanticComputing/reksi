...
def index():
    ...
    if input_data != None:
        if len(input_data) > 0:
            ...
            if code == 1:
                data = {"status":200,"data":results, 'service':"Regex Identifier Service", 'timestamp':dt.today().strftime('%Y-%m-%d %H:%M:%S'), "version":"version 1.1-beta"}
                return jsonify(data)
            else:
                data = {"status":-1,"Error":str(results), "service":"Regex Identifier Service", "timestamp":dt.today().strftime('%Y-%m-%d %H:%M:%S'), "version":"version 1.1-beta"}
                return jsonify(data)
        else:
